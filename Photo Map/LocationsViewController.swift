...
class LocationsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        //let venue = try! NSJSONSerialization.JSONObjectWithData(venueData, options: []) as? NSDictionary
        let location = venue["location"] as! NSDictionary
        let lat = location.valueForKey("lat") as? NSNumber
        let lng = location.valueForKey("lng") as? NSNumber
        let name = venue.valueForKey("name") as? String
        delegate.locationsPickedLocation(self, latitude: lat!, longitude: lng!, name: name!)
    }
